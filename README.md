...
| Tab | What it does |
| --- | --- |
| **Students** | Add a student (name, ID, class) and register their face with the phone camera or an uploaded photo. See who is present/absent today, attendance history per student, and export a date range to Excel / PDF. |
...
| POST | `/api/students` | Create student (`name`, `student_no`, `grade`, `photo`, `descriptors`) |
...
| GET | `/api/export/excel?from=&to=` | Excel workbook |
| GET | `/api/export/pdf?from=&to=` | PDF report |
